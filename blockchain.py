...
def get_balance(participant):
    ...
    print('things', tx_sender)
    amount_sent = functools.reduce(lambda tx_sum, tx_amt: tx_amt[0] if len(tx_amt) > 0 else 0 + tx_sum, tx_sender, 0)
    tx_recipient = [[tx['amount'] for tx in block['transactions'] if tx['recipient'] == participant] for block in blockchain]
    amount_received = functools.reduce(lambda tx_sum, tx_amt: tx_amt[0] if len(tx_amt) > 0 else 0 + tx_sum, tx_recipient, 0)
